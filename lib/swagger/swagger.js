			...
			methods.forEach(function (oneMethod) {
				...
				if (apiPath[route][oneMethod]) {
					...
					all_apis[oneMethod.toLowerCase()][soajsRoute] = {
						"_apiInfo": {
							"l": apiPath[route][oneMethod].summary || "empty",
							"group": (apiPath[route][oneMethod].tags) ? apiPath[route][oneMethod].tags[0] : "empty"
						},
	...
	"generateFiles": function (files, callback) {
		...
		async.each(files, function (fileObj, mCb) {
			...
			if (fileObj.purify) {
				...
				data = data.replace(/("group": "empty")/g, '"group": ""');
				data = data.replace(/("l": "empty")/g, '"l": ""');
