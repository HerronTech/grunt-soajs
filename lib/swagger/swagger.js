...
var swagger = {
	...
	"validateYaml": function (yamlJson) {
		...
		//loop in path
		Object.keys(yamlJson.paths).forEach(function(onePath){
			//loop in methods
			Object.keys( yamlJson.paths[onePath]).forEach(function(oneMethod  , key,  _array){
				if(!oneMethod.summary || oneMethod.summary === ""){
					throw new Error("Please enter a summary for API " + onePath + ": "+ oneMethod + " you want to build.");
				}
			});
		});
	},
